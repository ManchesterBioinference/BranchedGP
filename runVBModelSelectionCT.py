...
class objectiveBAndK:
    ...
    def __init__(self, Binit, mVBmodel, Ygene, pt, m):
        ...
        mVBmodel.kern.branchkernelparam.kern.lengthscales = np.max(
            np.array([m.kern[0].lengthscale.values, m.kern[1].lengthscale.values]))
        mVBmodel.kern.branchkernelparam.kern.variance = np.mean(
            np.array([m.kern[0].variance.values, m.kern[1].variance.values]))
        mVBmodel._compile()
    ...
    def f(self, theta):
        ...
        for i in range(n):
            ...
            VBHelperFunctions.InitialisePhiFromOMGP(
                self.mVBmodel, phiOMGP=self.m.phi, b=theta[
                    i, 0], Y=self.Ygene, pt=self.pt)
...
def InitModels(pt, XExpanded, Y, fMatern32):
    ...
    KbranchVB = bk.BranchKernelParam(kern, fm, BvInitial=np.ones((1, 1))) + GPflow.kernels.White(1) + \
        GPflow.kernels.Linear(1) + GPflow.kernels.Constant(1)  # other copy of kernel
...
def RunAnalysis(fMatern32, fTestRun, fUsePseudoTime):
    ...
    for g in genesToLookAt:
        ...
        m = OMGP(pt[:, None], Yg, K=2, variance=0.01, kernels=[komgp1, komgp2],
        ...
        _, phiInitial_invSoftmax, XExpanded = VBHelperFunctions.InitialisePhiFromOMGP(
            None, phiOMGP=m.phi, b=0, Y=Yg, pt=pt)
        ...
        eps = 1e-5
        bounds = [(l, u), (eps, 3 * Yg.var()), (eps, pt.max()), (eps, 3 * Yg.var()),
                  [0.1, 5], [0.1, 5]]  # B, lik var, len, var, lin var, c var
