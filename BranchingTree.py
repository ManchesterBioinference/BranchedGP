...
class BinaryBranchingTree:    
    ...
    def _printTree(self, node, ntabs):
        if(node != None):
            ...
            print '---------'*ntabs + str(node.idB) + '(' + str(node.val) + ')' + '[' + str(self.dictFunction[node.idB][0]) + ',' + str(self.dictFunction[node.idB][1]) + ']'
    ...
    def add(self, idParent, idB, val):        
        ...
        if(val < self.lbX or val > self.ubX):
            raise NameError('Value of node= ' + str(val) + ' outside bounds [' + str(self.lbX) +  ',' + str(self.ubX) + '] '  )
    ...
    def GetFunctionBranchTensor(self):
        ...
        for fi in range(m):
            ...
            for fj in range(m):           
                ...
                if(self.fDebug):
                    print 'Functions ' + str(fi) + ',' + str(fj) + ' crosspoints ' + str(branchpath) + ', values ' + str(branchvalues)            
        ...
        expectedBranchPoints = 1+np.array(range(nb)) 
    ...
    def GetConstrainedFunctionIndexList(self,Xin,idxOverwrite,labels):
        ...
        for ix,x in enumerate(Xin):
            ...
            if ix in idxOverwrite:
                ...
                for f in functionList:
                    XForKernel_Constrained_l.append([x, f+1]) # could have 1 or 0 based function list - does kernel care?
                    idx.append(inew)
                    inew = inew + 1
