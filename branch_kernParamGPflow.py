...
#     then need to do indexing self.Bs[bnan] in K
    ...
    def K(self, X, Y=None):
        ...
        if(self.fDebug):
            ...
            i1s = tf.Print(i1s_r,[tf.shape(i1s_r), i1s_r],message='i1s=',name='i1sdebug',summarize=snl) # will print message
            i2s = tf.Print(i2s_r,[tf.shape(i2s_r), i2s_r],message='i2s=',name='i2sdebug',summarize=snl) # will print message
        else:
            i1s = i1s_r
            i2s = i2s_r            
