...
for g in range(G):
    ...
    gpmodels.append(BranchedGP.FitBranchingModel.FitModel(Bsearch, GPt, GPy, globalBranching, maxiter=maxiter,
                                              fDebug=False, M=M))
    ...
    if fPlot:
        bplot.PlotBGPFit(GPy, GPt, Bsearch, gpmodels[g])
        bplot.savefig_mine('figs', 'syn%g' % g)
