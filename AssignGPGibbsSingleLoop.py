...
def checkAssignmentVector(indices,assignCheck,Xtrue):
    ...
    for i,ia in enumerate(assignCheck):
        ...
        if(ia not in indices[i]):
            ...
            strError = 'checkAssignmentVector: assignment vector: Element ' + str(i) + ' has value '  + str(ia) + ' but should be one of ' + str(indices[i])
...
def GetFunctionIndexListGeneral(Xin):
        ...
        Xnew = []
        inew = 0
        functionList = range(1,4) # can be assigned to any of root or subbranches, one based counting
...
class AssignGPGibbsFast(GPflow.gpr.GPR):
    def __init__(self, X, Y, kern, Z=None):
        ...
        assert X.shape[0] == Y.shape[0], "Must have enlarged X matrix. Dont use this function with only 1 function!  size of X is %g, Y is %g"%(X.shape[0], Y.shape[0])
    ...
    def CompileAssignmentProbability(self,fDebug=False,fMAP=True):
        ...
        with self.tf_mode():
            ...
            if self.ZExpanded is None:  # No sparse GP
                ...
                with tf.name_scope('CholeskyPrecomputationSparseGP'):
                    ...
                    self.KChol, self.Kuf, self.Kdiag = self._session.run([L, Kuf, Kdiag], feed_dict={self._free_vars: self.get_free_state()})
        if(fDebug):
        ...
        probVector = tf.Variable( np.zeros(NumberOfOverlappinFunctions,dtype=float)*np.NaN,tf.float64,name='probVector')
        ...
        for f in range(NumberOfOverlappinFunctions):
            ...
            with tf.name_scope('ComputeCoin_f_'+str(f)):
                # k'th function
                # bestAssignment[i] = k
                assignments_ind_tf = tf.Variable( tf.zeros([N],tf.int32) ,name='assind_'+str(f)) # this better be int32
                assignments_ind_tf = assignments_ind_tf.assign(assignments_tf) # initialise
    
                # bestAssignment[i] = k
                assignments_ind_tf = tf.scatter_update(assignments_ind_tf, pointToUpdate, tf.expand_dims(idxTF[f],0), name='initialass_'+str(f))
    
                if(fDebug):
                    assignments_ind_tf = tf.Print(assignments_ind_tf,[pointToUpdate, tf.shape(assignments_ind_tf), assignments_ind_tf],message='point - assignments_after update=',name='assignments_ind_tfddebug',summarize=10) # will print message
    
                #lik = tf.constant([2.],tf.float64) # for testing
                with self.tf_mode():
                    lik = self.build_likelihoodAssignment(assignments_ind_tf,fDebug=False)
                if(fDebug):
                    lik = tf.Print(lik,[pointToUpdate, tf.shape(lik), lik],message='point - lik=',name='likeddebug',summarize=10) # will print message
    
                probVector=tf.scatter_update(probVector, [f], lik, name='probVector'+str(f)) # without locking but ok to do in parallel but ik unique
                tf.histogram_summary('probvector', tf.squeeze(probVector))
        # print probability vector of all assignments
        if(fDebug):
            probVector = tf.Print(probVector,[pointToUpdate, tf.shape(probVector), probVector],message='point - probVector=',name='probVectordebug',summarize=5) # will print message
        ...
        if(fMAP):
            with tf.name_scope('MAPdecision'):  
                ...
                if(fDebug):
                    si = tf.Print(si,[pointToUpdate, tf.shape(si), si],message='siMAP point=',name='siMAPdebug',summarize=3) # will print message
                ...
                def assignmentProbabilityMAP(x,z,idx,iP):
                    ...
                    return self._session.run( (si,lastlik,probVector), feed_dict={self._free_vars: x,assignments_tf:z,idxTF:idx,pointToUpdate:iP})
            ...
            with tf.name_scope('GibbsDecision'):  
                ...
                if(fDebug):
                    si = tf.Print(si,[pointToUpdate, tf.shape(si), si],message='siGibbs point=',name='siGibbsdebug',summarize=3) # will print message
                lastlik = tf.slice(probVector, si, np.asarray([1],int), name='lastlik')
                init = tf.initialize_all_variables()
    ...
    def InferenceGibbsMAP(self,  fReturnAssignmentHistory=False,maximumNumberOfSteps=1, startingAssignment=None, fDebug=False,tol=1e-2):
        ...
        if(startingAssignment is None):
            ...
            checkAssignmentVector(self.indices, startingAssignment, self.XExpanded[startingAssignment,:]) # check we haven't messed up
        ...
        for c in range(maximumNumberOfSteps):
            for i in range(N): # could randomly permute data
                ...
                if(self.t[i] > Bv):
                    ...
                    (si,lastlik,sliceOut) = self._assignProbFunction(self.get_free_state(),bestAssignment,ind[1:],[i])
                    ...
                    if(fDebug):
                        checkAssignmentVector(self.indices, bestAssignment, self.XExpanded[bestAssignment,:]) # check we haven't messed up
    ...
    def build_likelihoodAssignment(self, assignments_tf, fDebug=False): # should be renamed as prior on assignments is included?
        if self.ZExpanded is None:  # No sparse GP
    ...
    def build_likelihoodAssignment_full(self, assignments_tf, fDebug=False): # should be renamed as prior on assignments is included?
        N = self.Y.shape[0]
        M = self.XExpanded.shape[0]
        L = self.KChol
        ...
        if(fDebug):
            ...
            assignments = tf.Print(assignments_tf,[tf.shape(assignments_tf), assignments_tf],message='assign=',name='assigndebug',summarize=snl) # will print message
        else:
        ...
        if(fDebug):
            LTA = tf.Print(LTA,[tf.shape(LTA), LTA],message='LTA=',name='LTAdebug',summarize=snl) # will print message
        ...
        D = self.Y.shape[1]
        if(fDebug):
            tauD = tf.Print(tau,[tf.shape(tau), tau],message='tau=',name='taudebug',summarize=snl) # will print message
            RD = tf.Print(R,[tf.shape(R), R],message='R=',name='Rdebug',summarize=snl) # will print message
            RiLPhiYD = tf.Print(RiLPhiY,[tf.shape(RiLPhiY), RiLPhiY],message='RiLPhiY=',name='RiLPhiYdebug',summarize=snl) # will print message
            tauD = tf.Print(tau,[tf.shape(tau), tau],message='tau=',name='taudebug',summarize=snl) # will print message
            ...
            return tf.Print(res,[tf.shape(res), res],message='lik=',name='likdebug',summarize=snl) # will print message
