...
from BranchedGP.MBGP.training_helpers import construct_and_fit_assigngp_model, SimplePhiConstructor, get_training_outcome, \
    ScipyOptimiser, ElvijsAmazingOptimiser, ElvijsRandomisingOptimiser, AssignGPOptimiser

_TRUE_BRANCHING_POINTS = [.2, .4, .5, .6]
_NUM_DATA_POINTS = 30
_BRANCHING_SEARCH_STARTING_POINTS = [.01, .15, .4, .55]  # Random selection; note how .4 is a true branching point
_MODEL_FIT_CONFIG = dict(M=0, maxiter=100, priorConfidence=0.65, kervar=1., kerlen=1., likvar=0.01)
_TOLERANCE = 0.01


@pytest.mark.parametrize("num_inputs", range(1, 5))
@pytest.mark.parametrize("method", ["L-BFGS-B"])  # The tolerance has to be relaxed in order for CG to pass
def test_demo(num_inputs: int, method: str) -> None:
    ...
    fit_details = FitModel(
        _BRANCHING_SEARCH_STARTING_POINTS, data.t, data.Y, data.state, **_MODEL_FIT_CONFIG,
...
def test_equivalence() -> None:
    ...
    fit_details = FitModel(
        [0.4], data.t, data.Y, data.state, **_MODEL_FIT_CONFIG,
        kern=kern,
    )
    trained_model2 = fit_details["model"]

    np.testing.assert_array_equal(trained_model1.BranchingPoints, trained_model2.BranchingPoints)
