...
def test_assigngp_defaults__to_kern_with_sigmoid_transform(simple_assigngp: AssignGP) -> None:
    """
...
def test_f_samples__is_not_smoking(num_samples: int, full_cov: bool, simple_assigngp: AssignGP) -> None:
    x_new = np.linspace(0, 1, 100)
    x_expanded, indices, _ = VBHelperFunctions.GetFunctionIndexListGeneral(x_new)

    samples = simple_assigngp.predict_f_samples(x_expanded, num_samples, full_cov=full_cov)
    assert samples.shape == (num_samples, x_expanded.shape[0], len(_TRUE_BRANCHING_POINTS))


# x_extended rows will always be divisible by 3, so let's check for unwanted broadcasting by adding 3 samples to the mix
@pytest.mark.parametrize("num_samples", [1, 43, 3])
def test_prior_samples__is_not_smoking(num_samples: int, simple_assigngp: AssignGP) -> None:
    x_new = np.linspace(0, 1, 100)
    x_expanded, indices, _ = VBHelperFunctions.GetFunctionIndexListGeneral(x_new)
...
def test_prior_samples_as_branched_data__is_not_smoking(num_data_points: int, simple_assigngp: AssignGP) -> None:
    ...
    np.testing.assert_array_equal(branched_data.branching_points, simple_assigngp.BranchingPoints)
...
def _simple_assigngp() -> AssignGP:
    ...
    phi_initial, phi_prior = GetInitialConditionsAndPrior(data.state, _PRIOR_CONFIDENCE, infPriorPhi=True)
    ...
    m = AssignGP(
        data.t, x_expanded, data.Y, indices,
        phiInitial=phi_initial, phiPrior=phi_prior, multi=True,
    )
    return m
