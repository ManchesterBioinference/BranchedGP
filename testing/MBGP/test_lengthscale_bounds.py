...
def test_lengthscale_prior_respected() -> None:
    ...
    data = ToyWigglyBranchedData([_TRUE_BP], num_data_points=_NUM_DATA_POINTS, wiggle_frequency=_WIGGLE_FREQUENCY)
    kern = get_branching_point_kernel(branching_points=_BP_SEARCH_STARTING_POINTS)
    fit_details = FitModel(_BP_SEARCH_STARTING_POINTS, data.t, data.Y, data.state, kern=kern, M=0)
    ...
    assert fit_details["model"].kernel.kern.lengthscales.numpy() >= lengthscale_lower_bound
...
def test_lengthscale_transformation_applied() -> None:
    ...
    kern = get_branching_point_kernel(branching_points=_BP_SEARCH_STARTING_POINTS, base_kernel=base_kernel)
