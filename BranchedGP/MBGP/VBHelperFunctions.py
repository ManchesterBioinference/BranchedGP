...
from .data_generation import Colours, DEFAULT_COLOURS


def plot_many_branching_posteriors(data, Bmode, Phi, prediction,
                                   fig=None, axa=None, num_figs=(2, 5), figsize=(10, 10),
                                   output_index=slice(None, None, None),
                                   fPlotVar=False, fPlotPhi=False, show_legend=False):
    """
    ...
    for (ib, branching_pointsi), ax in zip(enumerate(data.branching_points[output_index]), axa.flatten()):
        ...
        plotBranchModel(estimated_b, branching_pointsi, data.t, data.Y, ttestl, mul, varl,
                        Phi, fPlotVar=fPlotVar, d=ib, ax=ax, fColorBar=False, fPlotPhi=fPlotPhi,
...
def plotBranchModel(
    ...
    y_bounds = ax.axis()[-2:]
    ax.plot([estimatedB, estimatedB], y_bounds, '--m', linewidth=lw, label='Estimated BP')
    if trueB:
