...
def FitModel(bConsider, GPt, GPy, globalBranching, priorConfidence=0.80,
             M=10, likvar=1., kerlen=2., kervar=5., fDebug=False, maxiter=1000,
             fPredict=True, fixHyperparameters=False, optimisation_method="L-BFGS-B",
    ...
    assert GPt.ndim == 1
    assert GPy.ndim == 2
    assert GPt.size == GPy.shape[0], 'pseudotime and gene expression data must be the same size'
    assert globalBranching.size == GPy.shape[0], 'state space must be same size as number of cells'
    ...
    if(M == 0):
        ...
        m = AssignGP(GPt, XExpanded, GPy, indices, kern=kern,
                     phiInitial=phiInitial, phiPrior=phiPrior, multi=True)
    ...
    return {'loglik': ll, 'Bmode': branching_points[iw], 'Phi': Phi_l[iw], 'model': models[iw],
            'prediction': {'xtest': ttestl_l[iw], 'mu': mul_l[iw], 'var': varl_l[iw]},
            'hyperparameters': hyps[iw], 'posteriorB': postB, 'globalB': branching_points[0]}


def trainModel(gpflow_model, maxiter=100, method="L-BFGS-B"):
    LOG.info(f"Starting training. Initial loss: {gpflow_model.training_loss()}. "
    ...
    LOG.info(f"Training complete. Final loss: {gpflow_model.training_loss()}. "
             f"Model summary:\n{tabulate_module_summary(gpflow_model, 'simple')}")
