...
import gpflow
from gpflow import set_trainable, default_float, default_jitter
from gpflow.mean_functions import Zero
from gpflow.models.training_mixins import InputData

from . import VBHelperFunctions, BranchingTree as bt
...
def get_Bv(branching_vector, branching_index):
    ...
    return tf.expand_dims(tf.expand_dims(branching_vector[tf.squeeze(branching_index)], axis=0), axis=1)
...
class BranchKernelParam(gpflow.kernels.Kernel):
    ...
    def K(self, X, Y=None, dim=0):
            ...
            for fj in range(m):
                if (fi != fj):
                    with tf.name_scope("f" + str(fi) + "f" + str(fj)):  # scope
                        ...
                        t12F = tf.math.logical_and(f1F, f2F, name='F' + str(fi) + 'andF' + str(fj))
        ...
        if square:
            return K_s + tf.eye(tf.shape(K_s)[0], dtype=gpflow.default_float())*self.noise_level
        else:
            return K_s
...
class AssignGP(gpflow.models.model.GPModel, gpflow.models.InternalDataTrainingLossMixin):
    ...
    def _GetPhiBeta(self, dim=0):
        ...
        PhiBeta = tf.where(tf.squeeze(tf.greater(self.t, BranchedPt))[:,None], Phi, self.phiTrunk)
    ...
    def _GetPZ(self, dim=0):
        ...
        pZ = tf.where(tf.squeeze(tf.greater(self.t, BranchedPt))[:,None], self.eZ0, self.phiTrunk)
        # try squashing Phi to avoid numerical errors
        pZ = (1 - 2e-6) * pZ + 1e-6
        return pZ
    ...
    def __init__(self, t, XExpanded, Y, indices, phiPrior=None, phiInitial=None, logPhi=None,
                 fDebug=False, multi=False, sparse=True, kern: BranchKernelParam=None):
        ...
        if sparse:
            ...
            Z = np.linspace(0., 1., M, endpoint=False)
        ...
        self.logPhi = gpflow.Parameter(np.random.randn(t.shape[0], t.shape[0] * 3).astype(gpflow.default_float())) # 1 branch point => 3 functions
        ...
        if(phiPrior is None):
            ...
            phiPrior = np.c_[np.zeros(phiPrior.shape[0])[:, None], phiPrior]  # prepend 0 for trunk
        ...
        if logPhi is not None:
            ...
            for i, p in enumerate(self.t):
                self.phiTrunk[i, iterC:iterC + 3] = np.array([1 - 2 * eps, 0 + eps, 0 + eps])
                iterC += 3
        else:
    ...
    def InitialiseVariationalPhi(self, phiInitialIn):
        ...
        for i, p in enumerate(self.t):
            ...
            iterC += 3
        assert not np.any(np.isnan(phiInitial_invSoftmax)), 'no nans please ' + str(np.nonzero(np.isnan(phiInitialEx)))
    ...
    def _build_likelihood_multi_b(self):
        ...
        a1 = -0.5 * N * self.D * tf.math.log(2. * np.pi / tau)
        ...
        for dim in range(self.D):
            ...
            # Cholesky
            L = tf.linalg.cholesky(K_d) + tf.eye(M, dtype=gpflow.default_float()) * gpflow.default_jitter()
            ...
            # add everything up
            a2 += - 0.5 * tf.math.reduce_sum(tf.math.log(tf.math.square(tf.linalg.diag_part(R))))
            a4 += + 0.5 * tf.math.reduce_sum(tf.math.square(c))
            a5 += - self.build_KL(Phi_d, dim=dim)

        if self.fDebug:
            with tf.control_dependencies(print_all(dict(a1=a1, a2=a2, a3=a3, a4=a4, a5=a5))):
                ll = a1 + a2 + a3 + a4 + a5
        else:
            ll = a1 + a2 + a3 + a4 + a5
        return ll
    ...
    def _build_likelihood_sparse(self):
        ...
        a1 = -0.5 * N * self.D * tf.math.log(2. * np.pi * sigma2)  # -(N/2)log(2*pi*sigma2)
        a3 = - 0.5 * tf.math.reduce_sum(tf.math.square(self.Y)) / sigma2  # -( 1 /(2*sigma2) ) Y^TY

        a2, a4, a5, a6 = 0., 0., 0., 0.

        for dim in range(0, self.D):
            Kuu_d = self.kernel.K(self.ZExpanded, dim=dim) + tf.eye(M, dtype=gpflow.default_float()) * gpflow.default_jitter()
            ...
            P = tf.linalg.matmul(W, tf.transpose(W)) + tf.eye(M, dtype=gpflow.default_float())
            traceTerm = -0.5 * tf.math.reduce_sum(Kdiag_d * A_d) / sigma2 + 0.5 * tf.math.reduce_sum(tf.math.square(W))
            R = tf.linalg.cholesky(P)
            ...
            if (self.fDebug):
                ...
                traceTerm = tf.print(traceTerm, [traceTerm], name='traceTerm', summarize=10)
        if self.fDebug:
            with tf.control_dependencies(print_all(dict(a1=a1, a2=a2, a3=a3, a4=a4, a5=a5))):
    ...
    def _build_predict_multi_b(self, Xnew, full_cov=False):
        ...
        for dim in range(self.D):
            ...
            if full_cov:
                var = self.kernel.K(Xnew, dim=dim) + tf.linalg.matmul(tf.transpose(tmp2), tmp2)\
                    - tf.linalg.matmul(tf.transpose(tmp1), tmp1)
                shape = tf.stack([1, 1, tf.shape(Y_d)[1]])
                var = tf.tile(tf.expand_dims(var, 2), shape)
            else:
                var = self.kernel.K_diag(Xnew, dim=dim) + tf.math.reduce_sum(tf.math.square(tmp2), 0)\
    ...
    def build_KL(self, Phi, dim=0):
        return tf.math.reduce_sum(Phi * tf.math.log(Phi)) - tf.math.reduce_sum(Phi * tf.math.log(self._GetPZ(dim)))
    ...
    def predict_f_samples(
        ...
        if not full_cov:
            ...
            jittermat = tf.eye(N, batch_shape=[D], dtype=default_float()) * default_jitter()  # [D, N, N]
...
def get_branching_point_kernel(
    ...
    branching_point_locations = np.array(branching_points).flatten().astype(gpflow.default_float())
    bp_param = gpflow.Parameter(branching_point_locations, transform=transform, prior=prior)

    return BranchKernelParam(
        base_kern=base_kernel,
        branchPtTensor=get_branch_point_tensor(initial_branching_point=branching_point_locations.mean()),
